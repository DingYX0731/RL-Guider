...
def load_peptide_module(cache_dir):
    global MHC_peptide_predictor, EPS
    MHC_peptide_predictor = Class1PresentationPredictor.load(cache_dir)
    EPS = 1e-10
    

def load_protein_module(cache_dir, input_model_path): 
    global protein_model, protein_tokenizer
    device = "cuda"
    protein_model = load_ProteinDT_model(input_model_path, cache_dir, mean_output=True, num_labels=3)
...
def load_module(args):
    if args.task_id < 300:
        return True
    elif (args.task_id > 300 and args.task_id < 500):
        load_peptide_module(cache_dir=args.peptide_cache_dir)
        return True
    else:
        load_protein_module(cache_dir=args.protein_cache_dir, input_model_path=args.protein_model_path)
        return True
...
def load_thredhold(drug_type):
    if drug_type == 'peptide':
        f_threshold = open("/root/rl_guider/Data/peptide/peptide_editing_threshold.json", 'r')
...
def load_dataset(task_id, path):
    if task_id < 300:
        # with open("/root/rl_guider/data/small_molecule/small_molecule_editing.txt") as f:
        with open(path) as f:
        ...
        #################################################################################################################################
        f = open(path, "r") # path to peptide dataset
        #################################################################################################################################
        data = json.load(f)
        test_data = data[source_allele_type]
    elif task_id > 500:
        #################################################################################################################################
        data_dir = "/root/rl_guider/Data/protein/downstream_datasets"
        chache_dir = "/root/rl_guider/Data/protein/temp_pretrained_ProteinDT"
        #################################################################################################################################
